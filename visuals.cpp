...
void Visuals::SpreadCrosshair( ) { // i dont need this shit
	// dont do if dead.
	if( !g_cl.m_processing )
		return;
...
void Visuals::DrawProjectile( Weapon* ent ) {
	...
	Color col = settings.proj_color; // fuck this out
	col.a( ) = 0xb4;
...
void Visuals::DrawItem( Weapon* item ) {
	...
	Color col = g_menu.main.visuals.item_color.get( ); // fuckit
	col.a( ) = 0xb4;
...
void Visuals::DrawPlayer( Player* player ) {
	...
	if( box_esp )
		render::rect_outlined( box.x, box.y, box.w, box.h, to_color(settings.box_color), { 10, 10, 10, low_alpha } );
	...
	if( name_esp ) {
		...
		render::esp.string( box.x + box.w / 2, box.y - render::esp.m_size.m_height, to_color( settings.name_color ), name, render::ALIGN_CENTER );
	}
				...
				if( current ) {
					if( !( current->m_velocity.length_2d( ) > 0.1 && !current->m_fake_walk ) && data->m_body_index <= 3 ) {
						...
						if( width > 0.f ) {
							...
							clr.a( ) = alpha;
							render::rect( box.x + 1, box.y + box.h + 3, width, 2, to_color( settings.ammo_bar_color ) );
			...
			if( weapon ) {
				...
				if( data ) {
					...
					if( max != -1 && settings.ammo_esp ) {
						...
						clr.a( ) = alpha;
						render::rect( box.x + 1, box.y + box.h + 3 + offset, bar, 2, to_color( settings.ammo_bar_color ) );
