...
void Shots::OnHurt( IGameEvent *evt ) {
	...
	/* this is not right made, its improperly, we might do it later, now we do the aimbot stuff, update it and fix it */

	// we can chose to stop shooting at them.
	if ( mode == Resolver::Modes::RESOLVE_BODY && data->m_upd_index > 0 )
		--data->m_upd_index;

	else if ( mode == Resolver::Modes::RESOLVE_DELTA && data->m_delta_index > 0 )
		--data->m_delta_index;
