...
void AimPlayer::UpdateAnimations( LagRecord *record ) {
	...
	bool fake = !bot && settings.correct;
...
void AimPlayer::SetupHitboxes( LagRecord *record, bool history ) {
	...
	*/
	...
	if ( settings.body_aimm ) {
		...
		// prefer, fake.
		if ( record->m_mode != Resolver::Modes::RESOLVE_NONE && record->m_mode != Resolver::Modes::RESOLVE_WALK )
			m_hitboxes.push_back( { HITBOX_BODY, HitscanMode::PREFER } );
...
bool Aimbot::CheckHitchance( Player *player, const ang_t &angle ) {
	...
	CGameTrace tr;
	size_t     total_hits{ }, needed_hits{ ( size_t )std::ceil( ( settings.hitchance_amount * SEED_MAX ) / HITCHANCE_MAX ) };
...
bool AimPlayer::GetBestAimPosition( vec3_t &aim, float &damage, LagRecord *record ) {
	...
	if ( g_cl.m_weapon_id == ZEUS ) {
		...
		*/
		dmg = std::fmin( hp, settings.minimal_damage ); //settings.minimal_damage;		
		pendmg = std::fmin( hp, settings.penetrate_minimal_damage ); //settings.penetrate_minimal_damage;
